...
$$\text{Domain}\rightarrow\text{Cluster}\rightarrow\text{Application}\rightarrow\text{Scenario Class}\rightarrow\text{Metric Set}\rightarrow\text{Regime Classification}\rightarrow\text{Evidence Interface}$$
...
$$\mathcal{S}(A_j)=\mathcal{S}^{\mathrm{core}}_j\cup\mathcal{S}^{\mathrm{boundary}}_j\cup\mathcal{S}^{\mathrm{overlap}}_j$$
...
$$M_{j\ell}=\{m_1,m_2,\ldots,m_n\}_{j\ell}$$

The corresponding public metric vector is:

$$\mathbf{x}_{j\ell}(S_D)=\left(m_1(S_D),m_2(S_D),\ldots,m_n(S_D)\right)$$
...
$$(C_{j\ell},M_{j\ell},\rho_{j\ell})\rightarrow E_{j\ell}$$
